...
**Último Cambio:**
- [01 Aug 2026] `fix`: ajustado el rango del zoom anti-detección de 110%-112% a 105%-108% (petición del usuario). Resto del pipeline sin cambios. Reprobado con video real: duración y resolución correctas.
- [31 Jul 2026] `feat`: limpieza periódica de huérfanos en Supabase Storage (punto 3 de la revisión post-Railway, último de los tres). Nueva `database.py::obtener_archivos_en_uso()` (archivos referenciados por publicaciones `pendiente`/`procesando`) y `storage.py::limpiar_huerfanos()` (compara contra el listado real del bucket y borra lo que sobre; exige antigüedad mínima de 1h para no pisar una subida en curso, y no borra nada si no puede confirmar qué está en uso — falla seguro). Enganchado como job diario en `scheduler.py` (04:00, reutiliza el `AsyncIOScheduler` ya existente, sin infraestructura nueva). Notifica por Telegram si borra algo. Probado real: huérfano de prueba protegido por el margen de antigüedad, luego borrado sin margen, sin tocar 5 publicaciones pendientes legítimas que había en el bucket.
- [31 Jul 2026] `fix`: prevención de doble publicación durante un redeploy de Railway solapado (punto 2 de la revisión post-Railway; punto 3 — limpieza periódica de huérfanos en Storage — sigue pendiente, ver abajo). Nuevo estado `procesando` en el CHECK constraint `estado_valido` de `publicaciones` (migración manual en Supabase SQL Editor). `database.py::reclamar_publicacion()` hace un `UPDATE...WHERE estado='pendiente'` atómico (pendiente→procesando) antes de publicar; si otro proceso ya la reclamó, devuelve `False` y `scheduler.py` se salta esa publicación en vez de publicarla dos veces. Probado: dos intentos de reclamar la misma fila (solo el primero tiene éxito) y flujo completo real (claim → publish → `publicado`, `ig_media_id` confirmado).
- [31 Jul 2026] `fix`: prevención de archivo huérfano en Storage si el proceso muere entre subir y guardar en DB (punto 1 de la revisión post-Railway). `bot.py::cmd_add` reordenado: ahora inserta en la DB **antes** de subir a Storage (con `video_url=None`), sube después, y enlaza con la nueva `database.py::actualizar_video_url()`. Así, si el proceso cae a mitad de camino, queda un registro `pendiente` visible y gestionable desde Telegram (`/programados`, `/cancelar`) en vez de un archivo invisible en Storage que nada limpiaría. Si la subida falla, se marca error explícitamente. Probado end-to-end real: insert → estado intermedio confirmado (`video_url: None`) → subida → enlazado → limpieza vía `/cancelar` funcionando también en este nuevo orden.

**Historial Reciente:**
- [31 Jul 2026] `fix`: `/cancelar` dejaba huérfano el video en Supabase Storage — desde que `/add` sube a Storage inmediatamente al editar, cancelar una publicación pendiente solo marcaba `estado='cancelado'` en la DB sin borrar el archivo, comiéndose el 1GB gratis poco a poco. `cancelar_publicacion()` en `database.py` ahora devuelve el registro cancelado completo (antes solo `bool`); `bot.py::cmd_cancelar` usa ese registro para borrar también de Storage. Probado end-to-end real: crear → cancelar → confirmado que el archivo desaparece del bucket.
...
**Estado del Proyecto:**
- Sistema operativo 24/7 en Railway (no Oracle Cloud — decisión consciente del usuario, ver sección de Infraestructura). Publicando de verdad en Instagram, confirmado con `ig_media_id` real.
- Coste: ya no es 0€/mes (plan Hobby de Railway). Resto de servicios (Supabase, Telegram) siguen en tier gratuito.
